...
output "backend_servers_public_ips" {
  value = { for name, srv in module.backend_servers : name => srv.public_ip }
}

output "backend_servers_private_ips" {
  value = { for name, srv in module.backend_servers : name => srv.private_ip }
}
# Networking Outputs
output "vpc_id" {
  description = "VPC ID"
